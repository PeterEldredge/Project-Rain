...
namespace Aura2API
{
    ...
    public class Frustum
    {
        ...
        public void ComputeData()
        {
            ...
            _cameraRanges = new Vector4(0.0000000000000001f /*Set close to 0 but need non-zero for stereo projection matrices*/, Mathf.Min(_cameraComponent.farClipPlane, _frustumSettings.QualitySettings.farClipPlaneDistance));
            Shader.SetGlobalVector("Aura_FrustumRanges", _cameraRanges);
            _zParameters = new Vector4(-1.0f + _cameraComponent.farClipPlane / _cameraComponent.nearClipPlane, 1.0f);
            ...
            _computeDataComputeShader.SetVector("cameraPosition", new Vector4(_cameraComponent.transform.position.x, _cameraComponent.transform.position.y, _cameraComponent.transform.position.z, 1.0f));
            ...
            float[] frustumCornersWorldPositionArray = Vector4.zero.AsFloatArray();
            float[] secondaryFrustumCornersWorldPositionArray = Vector4.zero.AsFloatArray();
            if (_cameraComponent.GetCameraStereoMode() == StereoMode.SinglePass)
            {
                frustumCornersWorldPositionArray = _cameraComponent.GetFrustumCorners(Camera.MonoOrStereoscopicEye.Left, _cameraRanges.x, _cameraRanges.y).AsFloatArray();
                secondaryFrustumCornersWorldPositionArray = _cameraComponent.GetFrustumCorners(Camera.MonoOrStereoscopicEye.Right, _cameraRanges.x, _cameraRanges.y).AsFloatArray();
            }
            else
            {
                frustumCornersWorldPositionArray = _cameraComponent.GetFrustumCorners(Camera.MonoOrStereoscopicEye.Mono, _cameraRanges.x, _cameraRanges.y).AsFloatArray();
                secondaryFrustumCornersWorldPositionArray = Vector4.zero.AsFloatArray();
            }
            _computeDataComputeShader.SetFloats("frustumCornersWorldPositionArray", frustumCornersWorldPositionArray);
            _computeDataComputeShader.SetFloats("secondaryFrustumCornersWorldPositionArray", secondaryFrustumCornersWorldPositionArray);
            _computeAccumulationComputeShader.SetFloats("frustumCornersWorldPositionArray", frustumCornersWorldPositionArray);
            _computeAccumulationComputeShader.SetFloats("secondaryFrustumCornersWorldPositionArray", secondaryFrustumCornersWorldPositionArray);
            #endregion
            ...
            _computeDataComputeShader.SetFloats("previousFrameWorldToClipMatrix", _previousWorldToClipMatrix.ToFloatArray());
            _computeDataComputeShader.SetFloats("previousFrameSecondaryWorldToClipMatrix", _previousSecondaryWorldToClipMatrix.ToFloatArray());
            _computeDataComputeShader.SetVector("cameraRanges", _cameraRanges);
            ...
            if (_frustumSettingsToId.HasFlags(FrustumParameters.EnableVolumesTexture3DMask))
            {
                ...
                _computeDataComputeShader.SetVector("texture3DMaskAtlasTextureSize", new Vector4(AuraCamera.CommonDataManager.VolumesCommonDataManager.Texture3DMasksAtlas.width, AuraCamera.CommonDataManager.VolumesCommonDataManager.Texture3DMasksAtlas.height, AuraCamera.CommonDataManager.VolumesCommonDataManager.Texture3DMasksAtlas.depth, 0.0f));
            ...
            if (_frustumSettingsToId.HasFlags(FrustumParameters.EnableLightProbes))
            {
                ...
                _lightProbesCoefficientsTextureSizeVector = new Vector4(WorkingBuffers.LightProbesCoefficientsTextureResolution.x, WorkingBuffers.LightProbesCoefficientsTextureResolution.y, WorkingBuffers.LightProbesCoefficientsTextureResolution.z, 1.0f);
                _lightProbesCoefficientsTextureHalfTexelSize = new Vector4(1.0f / _lightProbesCoefficientsTextureSizeVector.x, 1.0f / _lightProbesCoefficientsTextureSizeVector.y, 1.0f / _lightProbesCoefficientsTextureSizeVector.z, 1.0f) * 0.5f;
            ...
            if (_cameraComponent.GetCameraStereoMode() == StereoMode.SinglePass)
            {
                ...
                _previousWorldToClipMatrix = _cameraComponent.GetWorldToClipMatrix(Camera.MonoOrStereoscopicEye.Left, _cameraRanges.x, _cameraRanges.y);
                _previousSecondaryWorldToClipMatrix = _cameraComponent.GetWorldToClipMatrix(Camera.MonoOrStereoscopicEye.Right, _cameraRanges.x, _cameraRanges.y);
            }
            else
            {
                _previousWorldToClipMatrix = _cameraComponent.GetWorldToClipMatrix(Camera.MonoOrStereoscopicEye.Mono, _cameraRanges.x, _cameraRanges.y);
                _previousSecondaryWorldToClipMatrix = Matrix4x4.identity;
            }
        ...
        public void SetFrustumGridResolution(Vector3Int frustumGridResolution)
        {
            ...
            _bufferResolutionVector = new Vector4(_frustumGridResolution.x, _frustumGridResolution.y, _frustumGridResolution.z, 1);
            _bufferTexelSizeVector = new Vector4(1.0f / _frustumGridResolution.x, 1.0f / _frustumGridResolution.y, 1.0f / _frustumGridResolution.z, 1);
