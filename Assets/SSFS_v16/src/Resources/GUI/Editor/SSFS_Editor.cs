...
public class SSFS_Editor : MaterialEditor
{
	...
	void SetBlendMode()
	{
		if ( m.HasProperty( "_Cull" ) ) m.SetInt( "_Cull" , ( twoSided && !billboard ) ? ( int )cull.Off : ( int )cull.Back );
	...
	public void DrawGeneralTab()
	{
		...
		FeatureToggle( "Face Camera ( Billboarding )" , "BILLBOARD" );
		if ( Graphics.activeTier == UnityEngine.Rendering.GraphicsTier.Tier3 )
		{

			EditorGUI.BeginChangeCheck();
			FeatureToggle( "Use Instancing" , "INSTANCING_ON" );
			if ( EditorGUI.EndChangeCheck() ) m.enableInstancing = ( bool )_features[ "INSTANCING_ON" ];
		}
	}
